...
10. **Fully managed gradient accumulation** (user request): the manual `for _ in range(gas)` micro-batch loop is removed. train.py now calls `engine.backward()` + `engine.step()` once per micro-batch — DeepSpeed's internal `micro_steps` counter gates the optimizer update (`is_gradient_accumulation_boundary()`, engine.py:3242), auto-scales the loss, and zeroes gradients at boundaries. `is_gradient_accumulation_boundary()` is sampled before `step()` (the counter increments at the end of `step()`, engine.py:3495). The `--grad-accum` argument is removed entirely: without `train_batch_size` in ds_config.json, GAS defaults to 1; set `train_batch_size` there to enable accumulation. Note: `run_train.sh` is kept exactly as the user edited it (hardcoded `--steps 10 --log-interval 1`, no `$@` pass-through) — do not restore.
